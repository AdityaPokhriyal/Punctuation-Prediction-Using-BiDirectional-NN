...
class LSTM(tf.keras.Model):
    ...
    def __init__(self, rng, x, n_hidden):
        ...
        self.LSTM_f = LSTMCell(rng=rng, n_in=n_hidden, n_out=n_hidden, minibatch_size=self.minibatch_size)
        self.LSTM_b = LSTMCell(rng=rng, n_in=n_hidden, n_out=n_hidden, minibatch_size=self.minibatch_size)

        # output model
        self.LSTM = LSTMCell(rng=rng, n_in=n_hidden*2, n_out=n_hidden, minibatch_size=self.minibatch_size)
        ...
        self.params += self.LSTM.params + self.LSTM_f.params + self.LSTM_b.params
    ...
    def call(self, inputs, training=None):
        ...
        def input_recurrence(initializer, elems):
            ...
            x_f_t = tf.cast(x_f_t, dtype=tf.int32)  # Cast indices to int32
            x_b_t = tf.cast(x_b_t, dtype=tf.int32)  # Cast indices to int32

            h_f_t = self.LSTM_f(inputs=(tf.nn.embedding_lookup(self.We, x_f_t), h_f_tm1))
            h_b_t = self.LSTM_b(inputs=(tf.nn.embedding_lookup(self.We, x_b_t), h_b_tm1))
            return [h_f_t, h_b_t]

        [h_f_t, h_b_t] = tf.scan(
            fn=input_recurrence,
            elems=[inputs, inputs[::-1]], # forward and backward sequences
            initializer=[self.LSTM_f.h0, self.LSTM_b.h0]
        )
        ...
        def output_recurrence(initializer, elems):
            ...
            h_t = self.LSTM(inputs=(x_t, h_tm1))
        ...
        [_, self.last_hidden_states, self.y] = tf.scan(
            ...
            initializer=[self.LSTM.h0, self.LSTM.h0, tf.zeros([self.minibatch_size, self.y_vocabulary_size])]
        )
